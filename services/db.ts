...
export const searchOrganizations = async (searchTerm: string): Promise<Organization[]> => {
    ...
    try {
        // Basic prefix search using Firestore range queries
        // Ideally use Algolia or Typesense for real text search
        const term = searchTerm.toLowerCase();
        const strFrontCode = term.slice(0, term.length - 1);
        const strEndCode = term.slice(term.length - 1, term.length);
        const endCode = strFrontCode + String.fromCharCode(strEndCode.charCodeAt(0) + 1);

        // Note: For this to work perfectly, we need a 'name_lower' field in DB. 
        // For prototype, we fetch limited list or exact code match.
        // Let's implement exact Code Match OR Name Search simulation
        
        const codeQ = query(orgsRef, where('code', '==', searchTerm.toUpperCase()));
        const codeSnap = await getDocs(codeQ);
        if (!codeSnap.empty) {
            return codeSnap.docs.map(d => ({id: d.id, ...d.data()} as Organization));
        }

        // Fetch all (Prototype only - do not use in prod with thousands of orgs)
        // In prod: Use a separate 'search_index' collection
        const allSnap = await getDocs(query(orgsRef, limit(20)));
        return allSnap.docs
...
export const createOrganization = async (name: string, owner: User, logoUrl: string): Promise<Organization | null> => {
    try {
        ...
        const newOrg: Organization = {
            ...
            name,
            logoUrl,
            coverUrl: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&q=80&w=1200', // Default
...
export const sendJoinRequest = async (userId: string, orgId: string, dept: DepartmentType, roleTitle?: string): Promise<boolean> => {
    try {
        ...
        if (!snap.empty) return true; // Already sent
...
export const approveJoinRequest = async (requestId: string, joinRequest: JoinRequest): Promise<boolean> => {
    try {
        await runTransaction(db, async (transaction) => {
            ...
            // 2. Update User Profile (Switch them to this org)
...
// ... [Keep existing Follow, User, Post, Course functions] ...
...
export const getUsersByDepartment = async (dept: DepartmentType, orgId: string): Promise<User[]> => {
  try {
    const q = query(membershipsRef, where('organizationId', '==', orgId), where('department', '==', dept), where('status', '==', 'ACTIVE'));
    const snap = await getDocs(q);
...
export const getFeedPosts = async (userDept: DepartmentType, orgId: string): Promise<FeedPost[]> => {
    try {
        const q = query(postsRef, where('organizationId', '==', orgId), limit(50));
        const snapshot = await getDocs(q);
...
export const getDailyTasks = async (dept: DepartmentType, orgId: string): Promise<Task[]> => {
  try {
    const q = query(tasksRef, where('organizationId', '==', orgId), where('department', '==', dept));
    const snapshot = await getDocs(q);
...
export const getCareerPathByDepartment = async (dept: DepartmentType, orgId: string): Promise<CareerPath | null> => {
    try {
        ...
        const snapshot = await getDocs(q);
        if (!snapshot.empty) return { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as CareerPath;
        return null;
    } catch (e) { return null; }
};
