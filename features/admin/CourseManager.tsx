...
import { Loader2, Plus, Globe, Lock, Edit2 } from 'lucide-react';
    ...
    <div className="bg-white border border-[#d8dfea] min-h-[500px]">
        ...
        <div className="bg-[#f7f7f7] border-b border-[#d8dfea] p-3 flex justify-between items-center">
            ...
            <button 
                ...
                className="bg-[#3b5998] text-white px-3 py-1 text-[11px] font-bold border border-[#29447e] flex items-center gap-1"
            >
        ...
        {loading ? (
            ...
            <div className="p-3">
                {courses.length === 0 && <p className="text-xs text-gray-500">Henüz eğitim oluşturulmamış.</p>}
                
                {courses.map(course => (
                    <div key={course.id} className="mb-4 pb-4 border-b border-[#e9e9e9] last:border-0 flex gap-4">
                        {/* Thumbnail */}
                        <div className="w-24 h-16 bg-[#f7f7f7] border border-[#ccc] shrink-0">
                            <img src={course.thumbnailUrl} className="w-full h-full object-cover" />
                        </div>

                        <div className="flex-1">
                            <h3 className="text-[13px] font-bold text-[#3b5998] hover:underline cursor-pointer mb-1">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => navigate('/admin/content', { state: { courseData: course } })} className="text-[10px] text-[#3b5998] hover:underline">Düzenle</button>
                                <button className="text-[10px] text-[#3b5998] hover:underline">Sil</button>
                            </div>
                        </div>
                    </div>
                ))}
